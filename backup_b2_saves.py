...
def replace_raw_item_key(data, key):
    ...
    item = rotate_data_right(
        xor_data(data[5:], old_key >> 5), old_key & 31)[2:]
...
def apply_structure(pbdata, s):
    ...
    for k, data in pbdata.items():
        ...
        if child_s is None:
            ...
            values = [apply_structure(read_protobuf(d[1]), child_s)
                      for d in data]
            fields[key] = values if repeated else values[0]
        else:
            raise Exception(
                "Invalid mapping %r for %r: %r" % (mapping, k, data))
    if len(raw) != 0:
        fields["_raw"] = {}
...
def remove_structure(data, inv):
    ...
    for k, value in data.items():
        ...
        if child_inv is None:
            ...
            raise Exception(
                "Invalid mapping %r for %r: %r" % (mapping, k, value))
    return pbdata
...
def read_xor32(src, p1, p2):
    v1 = src[p1] | (src[p1 + 1] << 8) | (
        src[p1 + 2] << 16) | (src[p1 + 3] << 24)
    v2 = src[p2] | (src[p2 + 1] << 8) | (
        src[p2 + 2] << 16) | (src[p2 + 3] << 24)
    return v1 ^ v2
...
def modify_save(data, changes, endian=1):
    ...
    if changes.has_key("backpack"):
        ...
        player[36][0][1] = write_repeated_protobuf_value(
            s[: 7] + [sdus] + s[8:], 0)
